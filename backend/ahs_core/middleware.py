...
class AHSSessionTokenMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response):
        super().__init__(get_response)
        engine = import_module(settings.AHS_SESSION_ENGINE)
        self.SessionStore = engine.SessionStore
    ...
    async def process_response(self, request, response) -> HttpResponse:
        try:
            ...
            return response
        logger.debug(f"process_response headers {response.headers}")
        logger.debug(f"process_response cookies {response.cookies}")
        # First check if we need to delete this cookie.
        # The session should be deleted only if the session is entirely empty.
        if settings.SESSION_COOKIE_NAME in request.COOKIES and empty:
            response.delete_cookie(
