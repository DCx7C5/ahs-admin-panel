...
class Command(RunserverCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=Path,
            help="Specify the SSL certificate file",
        )
    ...
    def inner_run(self, *args, **options):
        ...
        kpath = options.get('key')
        cpath = options.get('cert')

        def build_ssl_endpoint_description_strings(port, key_path, cert_path):
            return [f"ssl:{port}:privateKey={key_path}:certKey={cert_path}"]


        endpoints = build_ssl_endpoint_description_strings(self.port, kpath, cpath)
